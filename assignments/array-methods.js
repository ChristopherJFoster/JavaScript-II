...
const largeShirts = runners.filter(runner => runner.shirt_size === "L"); // Refactored for implicit return;
...
const ticketPriceTotal = runners.reduce(
  ...
  0
); // Refactored for implicit return;
...
// The community center wants send a special thank you letter to the runners who represent a business that donated $100 or more. Create an array that includes only such runners.
const bigSpenders = runners.filter(runner => runner.donation > 99); // Refactored for implicit return
console.log(bigSpenders);

// Problem 2

// A group of hackers wants to send spam emails to all the runners. These strange hackers don't care about the runners' names or anything else - all they want is an array of email addresses. Create the array.
const justEmailAddresses = runners.map(runner => runner.email);
console.log(justEmailAddresses);

// Problem 3
// It turns out that all shirts size large and above are unavailable - what a disaster! Get a list of runners who chose size large or above. Create an array named shirtContact that contains ONLY first_name, last_name, email, and shirt_size so those runners can be contacted later when their shirts arrive.
const shirtContact = runners.filter(runner => {
  delete runner.id;
  delete runner.company_name;
  delete runner.donation;
  return runner.shirt_size.includes("L");
});
console.log(shirtContact);
